  ...
  for (const call of callsToProcess) {
    try {
      ...
      const response = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          model: 'gpt-5.4-2026-03-05', // Use standard for backfill efficiency
          messages: [
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`[${correlationId}] Calling OpenAI (gpt-5.4-2026-03-05)`);
    ...
    try {
      ...
      if (updateError) {
        ...
        return new Response(
          JSON.stringify({ deal_heat: dealHeat, saved: false, error: 'Failed to save deal heat data' }),
          { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
    ...
    console.error(`[calculate-deal-heat] Error ${requestId}:`, error instanceof Error ? error.message : error);
    return new Response(
