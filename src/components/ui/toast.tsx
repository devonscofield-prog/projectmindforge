...
const toastVariants = cva(
  [
    "group pointer-events-auto relative flex w-full items-center justify-between gap-4 overflow-hidden rounded-lg border p-4 pr-8 shadow-lg",
    // Transitions
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Action
    ref={ref}
    className={cn(
      "inline-flex h-8 shrink-0 items-center justify-center rounded-md border bg-transparent px-3 text-sm font-medium ring-offset-background transition-all duration-200",
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      "absolute right-2 top-2 rounded-md p-1 text-foreground/50 opacity-0 transition-all duration-200",
      "group-hover:opacity-100 hover:text-foreground hover:scale-110 active:scale-95",
...
const ToastIcon = ({ variant }: { variant?: "default" | "destructive" | "success" | "warning" | "info" | null }) => {
  ...
  switch (variant) {
    case "success":
      return <CheckCircle className={cn(iconClass, "text-success-foreground")} />;
    case "destructive":
      return <AlertCircle className={cn(iconClass, "text-destructive-foreground")} />;
    case "warning":
      return <AlertTriangle className={cn(iconClass, "text-warning-foreground")} />;
    case "info":
