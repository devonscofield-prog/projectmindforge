 ...
 * Requires strategic_threading, critical_gaps, and objection_handling
 */
export function isStrategyAudit(value: unknown): value is StrategyAudit {
  if (!isObject(value)) return false;
  return 'strategic_threading' in value;
}
...
export function toCallTranscript(row: CallTranscriptRow, repName?: string | null): CallTranscript {
  return {
    ...
    notes: row.notes,
    analysis_status: row.analysis_status as CallTranscript['analysis_status'],
...
export function toCoachingSession(row: CoachingSessionRow): CoachingSession {
  return {
    ...
    source_call_id: (row as Record<string, unknown>).source_call_id as string | null ?? null,
    created_at: row.created_at,
    updated_at: row.updated_at,
  };
}
