...
function mapSession(data: any): SDRAssistantSession {
  return {
    id: data.id, user_id: data.user_id, title: data.title,
    is_active: data.is_active ?? false, created_at: data.created_at,
    updated_at: data.updated_at, messages: parseMessages(data.messages),
  };
}

export async function fetchSDRAssistantSession(userId: string): Promise<SDRAssistantSession | null> {
  const { data, error } = await (supabase.from as any)('sdr_assistant_sessions')
...
export async function saveSDRAssistantSession(userId: string, messages: SDRAssistantMessage[], sessionId?: string): Promise<boolean> {
  try {
    ...
    if (sessionId) {
      const { error } = await (supabase.from as any)('sdr_assistant_sessions')
        .update({ messages: messagesJson, title, updated_at: new Date().toISOString() }).eq('id', sessionId);
      if (error) throw error;
      return true;
    }

    const { data: existing } = await (supabase.from as any)('sdr_assistant_sessions')
...
export async function switchToSDRSession(userId: string, sessionId: string): Promise<boolean> {
  try {
    await (supabase.from as any)('sdr_assistant_sessions')
      .update({ is_active: false }).eq('user_id', userId).eq('is_active', true);
    const { error } = await (supabase.from as any)('sdr_assistant_sessions')
