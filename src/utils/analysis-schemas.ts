...
export const BehaviorScoreSchema = z.object({
  ...
  metrics: z.object({
    ...
    interactivity: z.object({
      ...
      status: z.enum(['Excellent', 'Good', 'Fair', 'Poor']),
    }).optional().describe("Interactivity score measuring turn-taking frequency"),
  }),
});
...
export const SalesAssetsSchema = z.object({
  recap_email: z.object({
    subject_line: z.string().describe("Email subject line with placeholders"),
    body_markdown: z.string().describe("Markdown formatted email body with required links and placeholders"),
  }),
  internal_notes_markdown: z.string().describe("CRM-ready internal notes in markdown format"),
  validation_warnings: z.array(z.string()).optional().describe("Any warnings about missing links or quality issues"),
});
...
export const CoachingSynthesisSchema = z.object({
  ...
  primary_focus_area: z.enum(['Discovery Depth', 'Behavioral Polish', 'Closing/Next Steps', 'Objection Handling', 'Strategic Alignment', 'Deal Progression']),
...
// --- 10. THE AUDITOR (Pricing Discipline) ---
export const PricingDisciplineSchema = z.object({
  discounts_offered: z.array(z.object({
    ...
    discount_value: z.string().describe("The actual discount offered"),
  ...
  coaching_tips: z.array(z.string()).describe("2-3 specific tips for improving pricing discipline"),
});

export type PricingDiscipline = z.infer<typeof PricingDisciplineSchema>;
